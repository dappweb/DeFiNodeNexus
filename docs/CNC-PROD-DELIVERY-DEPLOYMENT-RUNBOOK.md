...
SEPOLIA_RPC_URL=https://rpc.cncchainpro.com
...
for f in .env .env.local deploy/linux/.env.production; do
  sed -i "s|^CNC_NEXUS_ADDRESS=.*|CNC_NEXUS_ADDRESS=${NEW_NEXUS}|" "$f" || true
  sed -i "s|^CNC_SWAP_ADDRESS=.*|CNC_SWAP_ADDRESS=${NEW_SWAP}|" "$f" || true
